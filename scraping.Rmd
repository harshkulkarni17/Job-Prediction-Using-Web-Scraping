...
```{r}
Experience <- data.frame("Experience_in_years"= floor(runif(length(Company),min = 2, max = 5)))
Job_Data <- cbind(Job_Data, Experience)

Job_Data[grepl("Junior",Job_Data$Job_post),]$Experience_in_years = floor(runif(1,min = 0, max = 2))
Job_Data[grepl("Intern",Job_Data$Job_post),]$Experience_in_years = floor(runif(1,min = 0, max = 2))
Job_Data[grepl("Senior",Job_Data$Job_post),]$Experience_in_years = floor(runif(1,min = 5, max = 10))
Job_Data[grepl("Sr",Job_Data$Job_post),]$Experience_in_years = floor(runif(1,min = 5, max = 10))
Job_Data[grepl("Jr",Job_Data$Job_post),]$Experience_in_years = floor(runif(1,min = 0, max = 2))
...
skills <- data.frame(name = c("circuit design", "aptitude for maths", "system design", "hardware", "analog", "programming"))
...
for (i in 1:length(Company)){
  if(Job_Data$Job_post %like% "%Data Sci%"){
    each_skill <- sample_n(skills, 3, fac = "name")$name
    print(each_skill)
    Skills <- c(Skills,each_skill)
  }else if(Job_Data$Job_post %like% "%Software%"){
    each_skill <- sample_n(soft_skills, 3, fac = "name")$name
    Skills <- c(Skills,each_skill)
  }
}

# if(Job_post %like% "%Android%"){
#   random_index<-unique(floor(runif(3,min = 1, max = 6)))
#   st <- NULL
#   for (i in random_index){
#     st<- c(st,android_skills[i])
# }
#   Job_Data$Skills <- st
#   # print(st)
#   # Job_Data[grepl("Android",Job_Data$Job_post),]$Skills = st
# }
